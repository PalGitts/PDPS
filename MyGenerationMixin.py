    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            log_softmax = torch.log_softmax(next_token_logits, dim=-1) # PDPS            
            selected_token_wise_log_prob = log_softmax.gather(
                dim=-1, 
                index= next_tokens.unsqueeze(-1).to(log_softmax.device)
            ).squeeze(-1)
            
            track_log_probs.append(selected_token_wise_log_prob) # PDPS
            
            # PDPS
            if return_dict_in_generate:
                if output_scores:
                    scores = track_log_probs
